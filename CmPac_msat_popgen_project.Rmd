...
description <- "CmMsat_2017-03-21_EDA"
strata.file <- "Cm_Pac_strata_032117.csv"
genotypes.file <- "Cm_Pac_genodata_032117.csv"
...
Msat.Cm.geno.a<-Msat.Cm.geno[!(numNAs>((ncol(Msat.Cm.geno)-(NCOL(Msat.Cm.strata)-1))*0.75)),]
```
...
table(Msat.Cm.geno.a$SITE)
table(Msat.Cm.geno.a$Country)

#run if want to remove samples where have less than X in a strata or missing data for an entire locus:
Msat.Cm.geno.short<-subset(Msat.Cm.geno.a,SITE!="Costa Careyes")
Msat.Cm.geno.short<-subset(Msat.Cm.geno.short,SITE!="La Flor")
Msat.Cm.geno.short<-subset(Msat.Cm.geno.short,SITE!="Los Cobanos")
Msat.Cm.geno.short<-subset(Msat.Cm.geno.short,SITE!="Mata Oscura")
Msat.Cm.geno.short<-subset(Msat.Cm.geno.short,SITE!="Punta Amapala")

table(Msat.Cm.geno.short$SITE)

Msat.Cm.geno.short<-Msat.Cm.geno.short[,8:58]#remove extra columns so first one is LabID
```

###Create a diploid gtypes object####
```{r gtypes, include=TRUE}
Msat.g <- new("gtypes", gen.data = Msat.Cm.geno.short[, -1], ploidy = 2,
                     ind.names = Msat.Cm.geno.short[, 1], schemes = strata.schemes, strata="SITE")
